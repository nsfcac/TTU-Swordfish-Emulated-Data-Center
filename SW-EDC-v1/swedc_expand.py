   ...
   # n=int(input("How many nodes you need in your cluster: "))   
    ...
    for i in range(n2):
        if (network_name=="SWEDCnetwork"):
                    s= "docker container run -d -p "+str(5001+i+n1+n3)+":5000 --name sw-node"+str(i+n1+1+n3)+" --network SWEDCnetwork elham1296/sw-from-dockerfile:v2"	
        else:
                    s= "docker container run -d -p "+str(5001+i+n1+n3)+":5000 --name "+network_name+"-sw-node"+str(i+n1+1+n3)+" --network "+network_name+" elham1296/sw-from-dockerfile:v2"	       
